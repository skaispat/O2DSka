...
const MakeInvoice = () => {
  ...
  useEffect(() => {
    if (brokerSearch.trim() === "") {
      ...
      setFilteredBrokers(filtered.slice(0, 5)); // Show max 5 results
    }
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showBrokerDropdown && !event.target.closest('.broker-dropdown-container')) {
...
const fetchDropdownData = async () => {
  try {
    ...
    if (brokerJson.success && Array.isArray(brokerJson.data)) {
      ...
      for (let i = 0; i < Math.min(10, saudaData.length); i++) {
        if (saudaData[i] && Array.isArray(saudaData[i])) {
          ...
          for (let j = 0; j < row.length; j++) {
            ...
            if (cellValue.includes('broker name') || cellValue === 'broker name') {
              headerRowIndex = i;
              brokerNameIndex = j;
            }
            ...
            if (cellValue.includes('sauda no') || cellValue.includes('sauda number')) {
              saudaNoIndex = j;
            }
            if (cellValue.includes('order status') || cellValue.includes('status')) {
              orderStatusIndex = j;
            }
          }
      ...
      const uniqueBrokers = [...new Set(brokerNames)].filter(b => b && b.trim() !== '');
  ...
  const handleSubmitInvoice = async (orderNumber) => {
    if (!formData.billDate || !formData.billNo) {
      toast.error("Please fill Bill Date and Bill No");
      return;
    }
    ...
    try {
      ...
      // Use atomic insert to ensure thread-safe invoice number generation
      for (const row of rows) {
        ...
        const rowData = [
          ...
          currentItem.partyName,           // Column D: Party Name
        ...
        if (updateResult.generatedSN) {
          console.log("Generated Invoice Number:", updateResult.generatedSN);
        }
      }
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(value) =>
                        handleInputChange({ target: { name: "billStatus", value } })
                      }
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <div className="relative broker-dropdown-container">
                      <div
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white cursor-text flex items-center justify-between"
                        onClick={() => {
                      ...
                      >
                        <input
                          ...
                          placeholder={globalBrokerName || "Search and select broker..."}
                          className="flex-1 border-none outline-none bg-transparent text-sm"
                        />
                        <ChevronDown
                          size={16}
                          className={`text-gray-400 transition-transform ${showBrokerDropdown ? 'rotate-180' : ''}`}
                        />
                      </div>
                            ...
                            {filteredBrokers.length > 0 ? (
                              filteredBrokers.map((broker) => (
                                <div
                                  ...
                                  onClick={() => handleGlobalBrokerChange(broker)}
                                >
                                  {broker}
                                </div>
                              ))
