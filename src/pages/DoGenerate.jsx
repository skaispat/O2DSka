...
const transporterDropdownRef = useRef(null);
  ...
  const handleClickOutside = (event) => {
    ...
    if (transporterDropdownRef.current && !transporterDropdownRef.current.contains(event.target)) {
...
const addNewPartyToMaster = async (partyName) => {
  ...
  const handleAddNewParty = async () => {
    const currentSearchValue = document.querySelector('.party-select .ant-select-selection-search input')?.value;
    const partyToAdd = currentSearchValue || formData.partyName;

    if (partyToAdd && partyToAdd.trim() && !partyNames.includes(partyToAdd.trim())) {
   ...
   const handleAddNewTransporter = async () => {
    const currentSearchValue = document.querySelector('.transporter-select .ant-select-selection-search input')?.value;
    const transporterToAdd = currentSearchValue || selectedTransporter;

    if (transporterToAdd && transporterToAdd.trim() && !transporterName.includes(transporterToAdd.trim())) {
      const success = await addNewTransporterToMaster(transporterToAdd.trim());
      if (success) {
        setSelectedTransporter(transporterToAdd.trim());
      }
    } else if (transporterToAdd && transporterName.includes(transporterToAdd.trim())) {
...
const addNewTransporterToMaster = async (transporterName) => {
  try {
    ...
    const result = await response.json();
    if (!result.success) throw new Error(result.error || "Failed to add transporter");
...
const filteredBrandOptions = formData.brandName && formData.brandName.trim() !== ""
  ? brandOptions.filter(brand =>
    brand.toLowerCase().includes(formData.brandName.toLowerCase())
  )
  : brandOptions;

const filteredPartyOptions = formData.partyName && formData.partyName.trim() !== ""
  ? partyNames.filter(party =>
    party.toLowerCase().includes(formData.partyName.toLowerCase())
  )
  : partyNames;

const filteredTransporterOptions = selectedTransporter && selectedTransporter.trim() !== ""
  ...
  const handleAddNewBrand = async () => {
    const currentSearchValue = document.querySelector('.brand-select .ant-select-selection-search input')?.value;
    const brandToAdd = currentSearchValue || formData.brandName;

    if (brandToAdd && brandToAdd.trim() && !brandOptions.includes(brandToAdd.trim())) {
...
const addNewBrandToMaster = async (brandName) => {
  ...
  const filteredData = doData.filter((item) => {
    ...
    const matchesParty =
      filterParty === "all" || partyName === filterParty;
  ...
  {showPartyDropdown && (
    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
      {filteredPartyOptions.length > 0 ? (
        ...
        <div className="px-3 py-2 text-gray-500">No parties found</div>
      )}
      ...
      {formData.partyName &&
        ...
        !partyNames.includes(formData.partyName.trim()) && (
          <div
            onClick={() => addNewPartyToMaster(formData.partyName.trim())}
  ...
  {showTransporterDropdown && (
    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
      {filteredTransporterOptions.length > 0 ? (
        ...
        <div className="px-3 py-2 text-gray-500">No transporters found</div>
      )}
      ...
      {selectedTransporter &&
        ...
        !transporterName.includes(selectedTransporter.trim()) && (
          <div
            onClick={() => addNewTransporterToMaster(selectedTransporter.trim())}
            className="px-3 py-2 hover:bg-blue-100 cursor-pointer text-blue-600 border-t border-gray-200"
          >
            + Add "{selectedTransporter.trim()}" as new transporter
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      handleInputChange({ target: { name: "deliveryTerm", value } })
                    }
    ...
    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
      ...
      {formData.brandName &&
        ...
        !brandOptions.includes(formData.brandName.trim()) && (
          <div
            onClick={() => addNewBrandToMaster(formData.brandName.trim())}
            className="px-3 py-2 hover:bg-blue-100 cursor-pointer text-blue-600 border-t border-gray-200"
          >
